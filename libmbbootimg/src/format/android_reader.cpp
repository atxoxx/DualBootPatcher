...
static void advance_state(AndroidFormatCtx *const ctx)
{
    switch (ctx->state) {
    case State::START:
                       ...
                       uint64_t *offset_out)
{
    ...
    ptr = memmem(buf, buf_size, BOOT_MAGIC, BOOT_MAGIC_SIZE);
    if (!ptr) {
 ...
 *   * MB_BI_FATAL if any file operations fail fatally
 */
static int android_format_bid(MbBiReader *bir, void *userdata, int best_bid)
{
    AndroidFormatCtx *const ctx = static_cast<AndroidFormatCtx *>(userdata);
    int bid = 0;
    int ret;
...
static int android_format_read_header(MbBiReader *bir, void *userdata,
                                      MbBiHeader *header)
{
    AndroidFormatCtx *const ctx = static_cast<AndroidFormatCtx *>(userdata);
    int ret;
...
static int android_format_read_next_entry(MbBiReader *bir, void *userdata,
                                          MbBiEntry *entry)
{
    AndroidFormatCtx *const ctx = static_cast<AndroidFormatCtx *>(userdata);
    ...
    // Check truncation here instead of in android_format_read() so we can give
    // the caller an accurate size value
    ...
    // Integer overflow already checked in android_format_read_header()
...
static int android_format_read_data(MbBiReader *bir, void *userdata,
...
static int android_format_read_set_option(MbBiReader *bir, void *userdata,
...
static int android_format_free(MbBiReader *bir, void *userdata)
{
    (void) bir;
    AndroidFormatCtx *const ctx = static_cast<AndroidFormatCtx *>(userdata);
    free(ctx);
...
{
    AndroidFormatCtx *const ctx = static_cast<AndroidFormatCtx *>(
            calloc(1, sizeof(AndroidFormatCtx)));
    if (!ctx) {
        mb_bi_reader_set_error(bir, -errno,
                               "Failed to allocate AndroidFormatCtx: %s",
                               strerror(errno));
        return MB_BI_FAILED;
    }
    ...
    return _mb_bi_reader_register_format(bir,
                                         ...
                                         &android_format_bid,
                                         &android_format_read_set_option,
                                         &android_format_read_header,
                                         &android_format_read_next_entry,
                                         &android_format_read_data,
                                         &android_format_free);
}
